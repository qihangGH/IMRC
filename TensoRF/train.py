...
def render_test(args):
    ...
    if args.render_test:
        ...
        PSNRs_test = evaluation(test_dataset, tensorf, args, renderer, f'{logfolder}/{args.expname}/imgs_test_all/',
                                N_vis=-1, N_samples=-1, white_bg=white_bg, ndc_ray=ndc_ray, device=device)
        with open(os.path.join(f'{logfolder}/{args.expname}/imgs_test_all', f"psnrs.txt"), "w") as f:
            f.write(" ".join([str(v) for v in PSNRs_test]))
        with open(os.path.join(f'{logfolder}/{args.expname}/imgs_test_all', f"psnr.txt"), "w") as f:
            f.write("{}".format(np.mean(np.array(PSNRs_test))))
...
def reconstruction(args):
    ...
    if not args.ndc_ray:
        allrays, allrgbs, _ = tensorf.filtering_rays(allrays, allrgbs, bbox_only=True)
    ...
    for iteration in pbar:
        ...
        rgb_map, alphas_map, depth_map, _, _, _ = renderer(rays_train, tensorf, chunk=args.batch_size,
        ...
        if iteration in update_AlphaMask_list:
            ...
            if not args.ndc_ray and iteration == update_AlphaMask_list[1]:
                ...
                allrays, allrgbs, _ = tensorf.filtering_rays(allrays, allrgbs)
