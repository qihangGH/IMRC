...
class TensorBase(torch.nn.Module):
    ...
    @torch.no_grad()
    def filtering_rays(self, all_rays, all_rgbs, all_cam=None, N_samples=256, chunk=10240 * 5, bbox_only=False):
        ...
        if all_cam is not None:
            all_cam = all_cam[mask_filtered]
        print(f'Ray filtering done! takes {time.time() - tt} s. ray mask ratio: {torch.sum(mask_filtered) / N}')
        return all_rays[mask_filtered], all_rgbs[mask_filtered], all_cam
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, ndc_ray=False, N_samples=-1,
        ...
        if ndc_ray:
            ...
            xyz_sampled, z_vals, ray_valid = self.sample_ray(rays_chunk[:, :3], viewdirs, is_train=is_train,
                                                             N_samples=N_samples)
            # dtu's and blender's ray directions have been normalized, but llff's are not
            dists = torch.cat((z_vals[:, 1:] - z_vals[:, :-1], torch.zeros_like(z_vals[:, :1])), dim=-1)
        ...
        # TODO: learn density
        _weight = weight if learn_density else weight.detach()
        acc_map = torch.sum(_weight, -1)
        rgb_map = torch.sum(_weight[..., None] * rgb, -2)
        ...
        return rgb_map, depth_map, acc_map, rgb, weight, bg_weight  #sigma, alpha
