...
class TensoRFGrid(nn.Module):
    ...
    def scale_volume_grid(self, new_world_size):
        ...
        self.xy_plane = nn.Parameter(
            F.interpolate(self.xy_plane.data, size=[X, Y], mode='bilinear', align_corners=True))
        self.xz_plane = nn.Parameter(
            F.interpolate(self.xz_plane.data, size=[X, Z], mode='bilinear', align_corners=True))
        self.yz_plane = nn.Parameter(
            F.interpolate(self.yz_plane.data, size=[Y, Z], mode='bilinear', align_corners=True))
    ...
    def extra_repr(self):
        return f'channels={self.channels}, world_size={self.world_size.tolist()}, ' \
               f'n_comp={self.config["n_comp"]}, n_comp_xy={self.config.get("n_comp_xy", self.config["n_comp"])}'
