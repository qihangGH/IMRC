...
class DirectVoxGO(torch.nn.Module):
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, **render_kwargs):
        ...
        return ray_pts, ray_id, step_id, N_steps
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        ray_pts, ray_id, step_id, N_steps = self.sample_ray(
...
def get_training_rays(rgb_tr, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    ...
    cam_idx_tr = torch.zeros([len(rgb_tr), H, W]).long()
    imsz = [1] * len(rgb_tr)
    for i, c2w in enumerate(train_poses):
        ...
        cam_idx_tr[i] = i * torch.ones([H, W]).long()
        del rays_o, rays_d, viewdirs
    eps_time = time.time() - eps_time
    print('get_training_rays: finish (eps time:', eps_time, 'sec)')
    return rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, cam_idx_tr
...
def get_training_rays_flatten(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    ...
    cam_idx_tr = torch.zeros(N).long()
    imsz = []
    top = 0
    for cam_idx, (c2w, img, (H, W), K) in enumerate(zip(train_poses, rgb_tr_ori, HW, Ks)):
        ...
        cam_idx_tr[top:top + n] = cam_idx * torch.ones(n).long()
        imsz.append(n)
        top += n
