...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    assert poses.shape[-1] == len(depthfiles), 'Mismatch between imgs {} and poses {} !!!!'.format(len(depthfiles),
                                                                                                   poses.shape[-1])
...
def load_llff_data(basedir, factor=8, width=None, height=None,
    ...
    if spherify:
        ...
        render_poses = np.concatenate(
            [render_poses, np.broadcast_to(poses[0, :3, -1:], render_poses[:, :3, -1:].shape)], -1)
    ...
    poses[:, 1:3, 0] *= -1
    poses[:, 0, 1:3] *= -1
    render_poses[:, 1:3, 3] *= -1
    render_poses[:, 1:3, 0] *= -1
    render_poses[:, 0, 1:3] *= -1
