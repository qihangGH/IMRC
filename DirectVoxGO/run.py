...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        keys = ['rgb_marched', 'depth', 'alphainv_last', 'weight_marched']
        ...
        weight_marched = render_result['weight_marched'].cpu().numpy()


        with open(os.path.join(testsavedir, f"rgb{i}.png"), "wb") as imgout:
            Image.fromarray(np.array(
                (np.clip(rgb, 0., 1.) * 255.).astype(np.uint8))).save(imgout, "PNG")
        with open(os.path.join(testsavedir, f"depth{i}.png"), "wb") as imgout:
            depth = (depth - depth.min()) / (depth.max() - depth.min())
            Image.fromarray(np.array(
                (np.clip(depth[..., 0], 0., 1.) * 255.).astype(np.uint8))).save(imgout, "PNG")
        with open(os.path.join(testsavedir, f"acc{i}.png"), "wb") as imgout:
            Image.fromarray(np.array(
                (np.clip(weight_marched[..., 0], 0., 1.) * 255.).astype(np.uint8))).save(imgout, "PNG")

        rgbs.append(rgb)
        depths.append(depth)
        bgmaps.append(bgmap)
        accs.append(weight_marched)
    ...
    with open(os.path.join(testsavedir, f"psnrs.txt"), "w") as f:
        f.write(" ".join([str(v) for v in psnrs]))
    with open(os.path.join(testsavedir, f"psnr.txt"), "w") as f:
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage,
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, _ = dvgo.get_training_rays_in_maskcache_sampling(
            ...
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, _ = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
        else:
            rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, _ = dvgo.get_training_rays(
                rgb_tr=rgb_tr_ori,
                train_poses=poses[i_train],
                HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
...
if __name__ == '__main__':
    ...
    if args.render_test:
        ...
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=30,
                         quality=8)
        imageio.mimwrite(os.path.join(testsavedir, 'video.bgmap.mp4'), utils.to8b(1 - bgmaps / np.max(bgmaps)), fps=1,
                         quality=8)
