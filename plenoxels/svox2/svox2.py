...
class RenderOptions:
    ...
    random_sigma_std_background: float = 1.0  # Noise to add to sigma (for the BG model; only if randomize=True)
...
class SparseGrid(nn.Module):
    ...
    def __init__(
        ...
        init_links_inverse = torch.zeros(self.capacity, dtype=torch.int32, device=device)
        ...
        # Called sh for legacy reasons, but it's just the coefficients for whatever
        ...
        self.links_inverse: torch.Tensor
    ...
    def _fetch_links(self, links):
        '''
        Fetch the sigma and color data pointed by the links.
        :param links: torch.Tensor, 1-dim array with size (N,).
        '''
    ...
    def sample(self, points: torch.Tensor,
        ...
        if use_kernel and self.links.is_cuda and _C is not None:
            ...
            points_long = points.to(torch.long)
            for i in range(3):
                points_long[:, i].clamp_max_(self.links.size(i) - 2)
            wb = points - points_long
            wa = 1.0 - wb

            lx, ly, lz = points_long.unbind(-1)
    ...
    def _volume_render_gradcheck_lerp(self, rays: Rays, return_raylen: bool = False):
        ...
        while good_indices.numel() > 0:
            ...
            points_long = pos.to(torch.long)
            points_long.clamp_min_(0)
            points_long[:, 0] = torch.clamp_max(points_long[:, 0], gsz[0] - 2)
            points_long[:, 1] = torch.clamp_max(points_long[:, 1], gsz[1] - 2)
            points_long[:, 2] = torch.clamp_max(points_long[:, 2], gsz[2] - 2)
            pos -= points_long
        ...
        if self.use_background:
            ...
            for i in range(n_steps):
                ...
                points_long = points.to(torch.long)
                points_long[..., 0].clamp_max_(self.background_links.size(0) - 1)
                points_long[..., 1].clamp_max_(self.background_links.size(1) - 1)
                points_long[..., 2].clamp_max_(self.background_nlayers - 2)
    ...
    def _volume_render_gradcheck_nvol_lerp(self, rays: Rays, return_raylen: bool = False):
        ...
        while good_indices.numel() > 0:
            ...
            pos_long = pos.to(torch.long)
            pos_long.clamp_min_(0)
            pos_long[:, 0] = torch.clamp_max(pos_long[:, 0], gsz[0] - 2)
            pos_long[:, 1] = torch.clamp_max(pos_long[:, 1], gsz[1] - 2)
            pos_long[:, 2] = torch.clamp_max(pos_long[:, 2], gsz[2] - 2)
            pos -= pos_long

            # BEGIN CRAZY TRILERP
            lx, ly, lz = pos_long.unbind(-1)
            links000 = self.links[lx, ly, lz]
            links001 = self.links[lx, ly, lz + 1]
            links010 = self.links[lx, ly + 1, lz]
            links011 = self.links[lx, ly + 1, lz + 1]
    ...
    def volume_render_fused(
        ...
        self.sparse_grad_indexer = torch.zeros(
            (self.density_data.size(0),), dtype=torch.bool, device=self.density_data.device)
        ...
        if self.use_background:
            ...
            self.sparse_background_indexer = torch.zeros(
                list(self.background_data.shape[:-1]), dtype=torch.bool, device=self.background_data.device)
    ...
    def get_mask_out_test(
        ...
        grad_holder = _C.GridOutputGrads()
        grad_holder.grad_density_out = grad_density.clone()
        grad_holder.grad_sh_out = grad_sh.clone()
        grad_holder.mask_out = self.sparse_grad_indexer.clone()

        _C.get_mask_out_test(
            self._to_cpp(),
            rays._to_cpp(),
            self.opt._to_cpp(randomize=randomize),
            rgb_gt,
    ...
    def volume_render_image(
        ...
        if (self.basis_type != BASIS_TYPE_MLP and imrend_fn_name in _C.__dict__
                and not torch.is_grad_enabled() and not return_raylen):
    ...
    def volume_render_acc(self, rays: Rays):
        """
        Volumetric acc rendering for rays

        :param rays: Rays, (origins (N, 3), dirs (N, 3))

        :return: (N,)
        """
        return _C.volume_render_weight_term(
            self._to_cpp(),
            rays._to_cpp(),
            self.opt._to_cpp())
    ...
    def volume_render_acc_image(self, camera: Camera, batch_size: int = 5000):
        """
        Volumetric acc rendering for full image
        ...
        all_accs = []
        for batch_start in range(0, camera.height * camera.width, batch_size):
            accs = self.volume_render_acc(rays[batch_start: batch_start + batch_size])
            all_accs.append(accs)
        all_acc_out = torch.cat(all_accs, dim=0)
        return all_acc_out.view(camera.height, camera.width)
    ...
    def resample(
        ...
        with torch.no_grad():
            ...
            if use_weight_thresh:
                ...
                if max_elements > 0 and max_elements < max_wt_grid.numel() \
                    ...
                    weight_thresh_bounded = torch.topk(
                        max_wt_grid.view(-1), k=max_elements, sorted=False).values.min().item()
                ...
                if max_elements > 0 and max_elements < sample_vals_density.numel() \
                        and max_elements < torch.count_nonzero(sample_vals_mask):
                    # To bound the memory usage
                    sigma_thresh_bounded = torch.topk(
                        sample_vals_density.view(-1), k=max_elements, sorted=False).values.min().item()
            ...
            sample_vals_sh = (torch.cat(all_sample_vals_sh, dim=0) if len(all_sample_vals_sh)
            ...
            links_pos_valid = links_pos[init_links >= 0]
            init_links_inverse[links_valid.long()] = links_pos_valid
            self.links_inverse = init_links_inverse.to(device=device)
    ...
    def world2grid(self, points):
        ...
        scaling = self._scaling * gsz  # scaling = gsz / (2*radius), i.e., the number of grids per meter
    ...
    def save(self, path: str, compress: bool = False):
        ...
        data = {
            ...
            "links_inverse": self.links_inverse.cpu().numpy(),
    ...
    def load(cls, path: str, device: Union[torch.device, str] = "cpu"):
        ...
        links = z.f.links
        links_inverse = z.f.links_inverse
        ...
        grid.links_inverse = torch.from_numpy(links_inverse).to(device=device)
    ...
    def to_svox1(self, device: Union[torch.device, str, None] = None):
        ...
        for i in tqdm(range(n_refine)):
            t[index].refine()
        data_order = self.links.view(-1)[mask].long()
        t[index, :-1] = self.sh_data.data[data_order].to(device=device)
        t[index, -1:] = self.density_data.data[data_order].to(device=device)
        return t
    ...
    def tv(self, logalpha: bool = False, logalpha_delta: float = 2.0, ndc_coeffs: Tuple[float, float] = (-1.0, -1.0)):
        ...
        return _TotalVariationFunction.apply(
            self.density_data, self.links, 0, 1, logalpha, logalpha_delta, False, ndc_coeffs)
    ...
    def inplace_tv_grad(self, grad: torch.Tensor,
        ...
        if rand_cells is not None:
            ...
            _C.tv_grad(
                self.links, self.density_data, 0, 1, scaling,
                logalpha, logalpha_delta,
                False,
                ndc_coeffs[0], ndc_coeffs[1],
                grad)
    ...
    def inplace_tv_color_grad(
        ...
        if rand_cells is not None:
            ...
            _C.tv_grad(
                self.links, self.sh_data, start_dim, end_dim, scaling,
                logalpha,
                logalpha_delta,
                True,
                ndc_coeffs[0], ndc_coeffs[1],
    ...
    def optim_density_step(
            self, lr: float, beta: float = 0.9, epsilon: float = 1e-8, optim: str = 'rmsprop'):
        """
    ...
    def _to_cpp(self, grid_coords: bool = False, replace_basis_data: Optional[torch.Tensor] = None):
        ...
        gspec.links_inverse = self.links_inverse
        if grid_coords:
            gspec._offset = torch.zeros_like(self._offset)
            gspec._scaling = torch.ones_like(self._offset)
            gspec._offset_grid2world = torch.zeros_like(self._offset)
            gspec._scaling_grid2world = torch.ones_like(self._offset)
        else:
            gsz = self._grid_size()
            gspec._offset = self._offset * gsz - 0.5
            gspec._scaling = self._scaling * gsz
            gspec._offset_grid2world = - (self._offset * gsz - 0.5) / (self._scaling * gsz)
            gspec._scaling_grid2world = 1. / (self._scaling * gsz)
    ...
    def estimate_color_metric(
            ...
            images: torch.Tensor):
        '''
        :param c2ws: camera extrinsics, num x 4 x 4, camera to world.
        :param w2cs: camera extrinsics, num x 4 x 4, world to camera.
        :param intrinsics: camera intrinsics, num x 6 or 9, fx, fy, cx, cy, W, H, (NDC0, NDC1, 1.).
        :param images: images corresponding to each camera.
        '''
        color_estimated = None
        color_res = torch.zeros([len(self.density_data.data), ]).float().cuda()
        color_weight = torch.zeros([len(self.density_data.data), ]).float().cuda()
        if _C is not None and self.density_data.is_cuda:
            color_estimated = _C.estimate_color_metric(
                self._to_cpp(), self.opt._to_cpp(), c2ws, w2cs, intrinsics, images, color_weight, color_res)
        else:
            print('To be implemented using torch functions.')
        return color_estimated, color_weight, color_res
