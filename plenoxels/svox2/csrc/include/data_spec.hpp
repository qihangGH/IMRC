...
struct SparseGridSpec {
  ...
  Tensor _offset;
  Tensor _scaling;
  Tensor _offset_grid2world;
  Tensor _scaling_grid2world;
  ...
  inline void check() {
    ...
    CHECK_CPU_INPUT(_offset_grid2world);
    CHECK_CPU_INPUT(_scaling_grid2world);
    TORCH_CHECK(density_data.ndimension() == 2);
    TORCH_CHECK(sh_data.ndimension() == 2);
    TORCH_CHECK(links.ndimension() == 3);
    TORCH_CHECK(links_inverse.ndimension() == 1);
  }
};
