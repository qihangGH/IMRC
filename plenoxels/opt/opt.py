...
group.add_argument('--train_dir', '-t', type=str, default='ckpt/test',
...
# grid = svox2.SparseGrid(reso=256,
#                         center=[0., 0., 0.],
#                         radius=1.28,
...
# ii, jj, kk = np.arange(256), np.arange(256), np.arange(64)
# i, j, k = np.meshgrid(ii, jj, kk, indexing='ij')
# grid_coord = np.stack([i, j, k], axis=-1).astype(np.int32)
# ndc = grid.grid2world(torch.from_numpy(grid_coord).to(device).float()).cpu().numpy()
# near = 1.0
# ndc_coeffs = dset.ndc_coeffs
# z = 2 * near / (1 - ndc[..., 2])
# x = ndc[..., 0] * z / ndc_coeffs[0]
# y = ndc[..., 1] * z / ndc_coeffs[1]
# ray_d = np.stack([x[..., 0], y[..., 0], z[..., 0]], axis=-1) / z[0, 0, 0]
# print(0)

# sdf = np.load(r'/lkq/lkq/IBRNet/data/huaizhang_test/sdf/sdf_vol_0.npy')
# scale1 = 50.
# scale2 = 100.
# sdf = scale1 * np.exp(-scale2 * np.abs(sdf))
# sdf[sdf < 5] = 0.
# sdf = sdf[:, ::-1, ::-1]
# sdf = np.transpose(sdf, [1, 2, 0])[..., ::-1]
# sdf = np.transpose(sdf, [1, 0, 2])[:, ::-1]
# sdf = np.transpose(sdf, [2, 0, 1])

# DC -> gray; mind the SH scaling!
grid.sh_data.data[:] = 0.0
grid.density_data.data[:] = 0.0 if args.lr_fg_begin_step > 0 else args.init_sigma
# grid.density_data.data[:] = torch.from_numpy(np.load('ibrnet_density_fern.npy').reshape([-1, 1])).to(device).float()

# mask = grid.links >= 0
# grid.density_data.data[:] = 0.0
# grid.density_data.data[grid.links[mask].long()] = \
#     torch.from_numpy(sdf[mask.cpu()][..., None]).to(mask.device).float()
...
grid.save(ckpt_path + '_init', compress=True)
