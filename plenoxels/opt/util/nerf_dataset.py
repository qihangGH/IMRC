...
class NeRFDataset(DatasetBase):
    ...
    def __init__(
        ...
        if split == 'train' and kwargs['train_hold_every'] is not None:
            all_c2w = all_c2w[::int(kwargs['train_hold_every'])]
            all_gt = all_gt[::int(kwargs['train_hold_every'])]
            print(f'Train with sparse view: {len(all_gt)}')

        if split == 'test' and kwargs['test_hold_every'] is not None:
            all_c2w = all_c2w[::int(kwargs['test_hold_every'])]
            all_gt = all_gt[::int(kwargs['test_hold_every'])]
            print(f'Test with sparse view: {len(all_gt)}')
        ...
        if self.gt.size(-1) == 4:
            if kwargs["high_fq"]:
                mask = (self.gt[..., 3:] > 0).float()
                if white_bkgd:
                    gt = self.gt[..., :3] * self.gt[..., 3:] + (1.0 - self.gt[..., 3:])
                    gt_inv = gt * mask
                else:
                    gt = self.gt[..., :3]
                    gt_inv = gt + (1. - mask)
                    gt_inv[gt_inv > 1.] = 1.
                gts = [gt[i] if i % 2 == 0 else gt_inv[i] for i in range(len(gt))]
                self.gt = torch.stack(gts).float()
