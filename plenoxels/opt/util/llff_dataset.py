...
import imageio
import cv2
from .util import Rays, Intrin, get_nearest_pose_ids
...
class LLFFDataset(DatasetBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.split == "train":
            # TODO: sparse views, gt, c2w, imgs
            # selected_ids = get_nearest_pose_ids(self.c2w[0].cpu().numpy(), self.c2w.cpu().numpy(), len(self.c2w))
            # selected_ids = selected_ids[::3]
            # self.c2w = self.c2w[selected_ids]
            # self.gt = self.gt[selected_ids]
            # self.imgs = [self.imgs[i].copy() for i in selected_ids]
            # self.n_images = len(selected_ids)
    ...
    def _load_images(self):
        ...
        # np.save('/lkq/lkq/IBRNet/data/nerf_llff_data/room/render_c2w.npy', self.render_c2w.numpy())
