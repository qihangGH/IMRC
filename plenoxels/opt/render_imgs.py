...
parser.add_argument('--use_cmap', default=False, action='store_true')
...
print('Bkgd brightness', grid.opt.background_brightness)
...
with torch.no_grad():
    ...
    for img_id in tqdm(range(0, n_images, img_eval_interval)):
        ...
        acc_im = grid.volume_render_acc_image(cam)
        depth_im = grid.volume_render_depth_image(cam)
        depth_im = (depth_im - torch.min(depth_im)) / (torch.max(depth_im) - torch.min(depth_im))
        if args.ray_len:
        ...
        acc_img_path = path.join(render_dir, f'acc{img_id:04d}.png')
        depth_img_path = path.join(render_dir, f'depth{img_id:04d}.png')
        im = im.cpu().numpy()
        acc_im = acc_im.cpu().numpy()
        ...
        if not args.timing:
            ...
            acc_im = (acc_im * 255).astype(np.uint8)
            depth_im = (depth_im * 255).astype(np.uint8)

            if args.use_cmap:
                depth_im_colored = plt.cm.viridis(depth_im)[..., :-1]
                depth_im_colored = (depth_im_colored * 255).astype(np.uint8)

            if not args.no_imsave:
                imageio.imwrite(img_path, im)
                imageio.imwrite(acc_img_path, acc_im)
                imageio.imwrite(depth_img_path, depth_im)
                if args.use_cmap:
                    depth_colored_img_path = path.join(render_dir, f'depth{img_id:04d}_colored.png')
                    imageio.imwrite(depth_colored_img_path, depth_im_colored)
            if not args.no_vid:
                frames.append(im)
        im = None
        n_images_gen += 1
    if want_metrics:
        ...
        with open(path.join(render_dir, f"psnrs.txt"), "w") as f:
