...
from util.cf_color_utils import Grid, calc_color_metric
utils.MAX_SH_BASIS = 16


parser = argparse.ArgumentParser()
config_util.define_common_args(parser)
parser.add_argument('density_path', type=str)
...
print("Density path:", args.density_path)
density_volume = np.load(args.density_path)
density_volume = torch.from_numpy(density_volume).float()
suffix = f'{density_volume.shape[0]}_{args.basis_dim}'
if args.high_fq:
    suffix += '_high_fq'

if args.step is not None:
    suffix = str(args.step) + '_' + suffix
# print(suffix)
print("Density volume's shape:", density_volume.shape)
...
grid = Grid(
    density_volume,
    reso=list(density_volume.shape),
...
with torch.no_grad():
    ...
    with open(os.path.join(os.path.dirname(args.density_path), f"milisecs_{suffix}.txt"), "w") as f:
...
def test_psnr(split='test'):
    save_dir = os.path.join(os.path.dirname(args.density_path), split + f'_{suffix}')
...
if __name__ == '__main__':
    ...
    np.savetxt(os.path.join(
        os.path.dirname(args.density_path),
        f'IMRC_{suffix}.txt'
    ),
        np.array([metric]))
    ...
    np.save(os.path.join(os.path.dirname(args.density_path), f'color_res_vol_norm2_{suffix}.npy'), color_res_volume)
    # np.save(os.path.join(os.path.dirname(args.density_path), f'color_weight_vol_{suffix}.npy'), color_weight_volume)
    print(f'IMRC: {metric}')
