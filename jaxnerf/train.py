...
def train_step(model, rng, state, batch, lr):
    ...
    def loss_fn(variables):
        ...
        rgb, unused_disp, unused_acc, trans = ret[-1]
        trans_coef = FLAGS.trans_coef  # 1e-2
        if trans_coef > 0 and "masks" in batch:
            # transmittance loss
            loss = (
                    (rgb - batch["pixels"][Ellipsis, :3]) ** 2 +
                    trans_coef * (trans[:, None] - batch["masks"][Ellipsis, :3]) ** 2
            ).mean()
        else:
        ...
        if len(ret) > 1:
            ...
            rgb_c, unused_disp_c, unused_acc_c, trans_c = ret[0]
            if trans_coef > 0 and "masks" in batch:
                loss_c = (
                        (rgb_c - batch["pixels"][Ellipsis, :3]) ** 2 +
                        trans_coef * (trans_c[:, None] - batch["masks"][Ellipsis, :3]) ** 2
                ).mean()
            else:
...
def main(unused_argv):
    ...
    # Resume training the step of the last checkpoint.
    ...
    for step, batch in zip(range(init_step, FLAGS.max_steps + 1), pdataset):
        ...
        if FLAGS.render_every > 0 and step % FLAGS.render_every == 0:
            ...
            if jax.host_id() == 0:
                test_case["pixels"] = test_case["pixels"][..., :3]
