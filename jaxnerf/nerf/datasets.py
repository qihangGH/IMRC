...
class Blender(Dataset):
    ...
    def _load_renderings(self, args):
        ...
        self.render_poses = np.load(os.path.join(args.data_dir, f'render_c2w.npy'))
...
class DTU(Dataset):
    ...
    def _load_renderings(self, args):
        ...
        camera_dict = np.load(os.path.join(args.data_dir, self.render_cameras_name))
        self.camera_dict = camera_dict
        images_lis = sorted(glob(os.path.join(args.data_dir, 'image/*.png')))
        masks_lis = sorted(glob(os.path.join(args.data_dir, 'mask/*.png')))
        n_imgs = len(images_lis)

        scan = args.data_dir.split('/')[-1][8:]
        self.render_poses = np.load(os.path.join(args.data_dir, f'render_c2w_{scan}.npy'))

        i_test = np.arange(n_imgs)[::args.llffhold][1:-1]
        ...
        for i in id_list:
            ...
            # for adding transmittance loss
            # image = np.concatenate([image, 1. - mask], axis=-1)
            ...
            if args.factor == 2:
                ...
                raise ValueError("DTU dataset only supports factor=0 or 2, {} "
                                 "set.".format(args.factor))
            cams.append(pose)
            images.append(image)
            masks.append(1. - mask)
    ...
    def _train_init(self, args):
        ...
        if args.batching == "all_images":
            self.images = self.images.reshape([-1, 3])
            self.masks = self.masks.reshape([-1, 3])
            self.rays = utils.namedtuple_map(lambda r: r.reshape([-1, r.shape[-1]]),
                                             self.rays)
        elif args.batching == "single_image":
            self.images = self.images.reshape([-1, self.resolution, 3])
            self.masks = self.masks.reshape([-1, self.resolution, 3])
            self.rays = utils.namedtuple_map(
                lambda r: r.reshape([-1, self.resolution, r.shape[-1]]), self.rays)
        else:
            raise NotImplementedError(
    ...
    def _generate_rays(self):
        """
        Generating rays for all images.
        The DTU dataset is in opencv convention, and f_x, f_y, c_x, and c_y are different.
        """
        if self.split == "test":
            n_render_poses = self.render_poses.shape[0]
        ...
        x, y = np.meshgrid(  # pylint: disable=unbalanced-tuple-unpacking
            ...
            np.arange(self.h, dtype=np.float32) + pixel_center,  # Y-Axis (rows)
            indexing="xy")
        camera_dirs = np.stack([(x - self.cx) / self.fx,
                                (y - self.cy) / self.fy, np.ones_like(x)],
                               axis=-1)
        directions = ((camera_dirs[None, Ellipsis, None, :] *
                       self.camtoworlds[:, None, None, :3, :3]).sum(axis=-1))
        origins = np.broadcast_to(self.camtoworlds[:, None, None, :3, -1],
    ...
    def _next_train(self):
        ...
        if self.batching == "all_images":
            ray_indices = np.random.randint(0, self.rays[0].shape[0],
                                            (self.batch_size,))
            batch_pixels = self.images[ray_indices]
            batch_masks = self.masks[ray_indices]
            batch_rays = utils.namedtuple_map(lambda r: r[ray_indices], self.rays)
        elif self.batching == "single_image":
            image_index = np.random.randint(0, self.n_examples, ())
            ray_indices = np.random.randint(0, self.rays[0][0].shape[0],
            ...
            raise NotImplementedError(
                f"{self.batching} batching strategy is not implemented.")
        return {"pixels": batch_pixels, "masks": batch_masks, "rays": batch_rays}
