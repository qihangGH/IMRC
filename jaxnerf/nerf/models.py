...
class NerfModel(nn.Module):
    ...
    def __call__(self, rng_0, rng_1, rays, randomized):
        ...
        comp_rgb, disp, acc, weights, trans = model_utils.volumetric_rendering(
            rgb,
            sigma,
            z_vals,
            rays.directions,
        ...
        if self.num_fine_samples > 0:
            ...
            rgb = self.rgb_activation(raw_rgb)
            sigma = self.sigma_activation(raw_sigma)
            comp_rgb, disp, acc, unused_weights, trans = model_utils.volumetric_rendering(
                rgb,
                sigma,
                z_vals,
                rays.directions,
            ...
            ret.append((comp_rgb, disp, acc, trans))
        return ret
