...
def define_flags():
    ...
    # Export density
    flags.DEFINE_bool("export_with_fine", True, "export the density with MLP fine.")
    flags.DEFINE_integer("export_chunk_size", 524288, "export chunk size; reduce it to avoid an OOM error")
    flags.DEFINE_integer("step", None, "step number to load or None to load latest")
    flags.DEFINE_string("reso",
                        "512,512,512",
                        "the resolution of the grid")
    flags.DEFINE_string("radius",
                        "1.0,1.0,1.0",
    ...
    flags.DEFINE_bool(
        "white_bkgd", False, "using white color as default background."
    ...
    flags.DEFINE_bool(
        "render_path",
        False,
        "render generated path if set true."
    ...
    # Train Flags
    flags.DEFINE_float("trans_coef", 0., "the coefficient of the transmittance loss")
...
def render_image(render_fn, rays, rng, normalize_disp, chunk=8192):
    ...
    rgb, disp, acc, _ = [jnp.concatenate(r, axis=0) for r in zip(*results)]
